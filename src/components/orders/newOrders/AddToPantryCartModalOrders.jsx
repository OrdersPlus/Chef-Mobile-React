...
import { errorToast } from "../../custom/Toastify";
...
const AddToPantryCartModalOrders = ({ modalData, sections }) => {
  ...
  const handleAddToPantry = async () => {
    ...
    if (!selectedSection) {
      errorToast("Please select a section.");
      return;
    }
      await postAxios(
        `${
          import.meta.env.VITE_BACK_END_URL
        }orders/add-to-pantry/${selectedSection}`,
        setLoader,
        payload
      );
    }
            ...
            <div className="w-full sm:w-[60%] space-y-1 text-sm text-center">
              ...
              <div className="flex items-center space-x-2 mt-2 justify-center">
                ...
                <span className="text-lg font-semibold text-gray-900">
                  ${modalData?.cost_price}/{modalData?.unit_of_measurement}
                </span>
              </div>
              <select className="w-full px-4 py-2 border-2 border-white rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 mb-4 shadow-xl">
